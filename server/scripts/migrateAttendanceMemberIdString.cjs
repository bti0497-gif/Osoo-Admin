...
async function main() {
  ...
  if (!memberId) throw new Error('attendance.member_id 컬럼을 찾을 수 없습니다.');
  ...
  if (String(memberId.type).toUpperCase() === 'STRING') {
    console.log('이미 STRING 타입이므로 마이그레이션이 필요 없습니다.');
    return;
  }
  ...
  console.log(`STRING 스키마 테이블 생성 완료: ${DATASET_ID}.${TABLE_ID}`);
