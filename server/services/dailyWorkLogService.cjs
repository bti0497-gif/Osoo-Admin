...
function getMonthEndDate(dateStr) {
  const d = new Date(`${String(dateStr).slice(0, 7)}-01T00:00:00`);
...
function enumerateMonthsInRange(startDate, endDate) {
  const out = [];
  const current = new Date(`${startDate.slice(0, 7)}-01T00:00:00`);
...
async function syncCertificateCacheForRange(db, startDate, endDate) {
  ...
  for (const monthDate of months) {
    await syncCertificateCacheForMonth(db, monthDate);
  }
}
...
function buildBindingsForDate(db, date) {
  ...
  const setQualitySlot = (slotIndex, row, isTargetDate) => {
    bindings[`수질날짜${slotIndex}`] = isTargetDate && row ? normalizeDate(row.report_date) : '';
    bindings[`수질ph${slotIndex}`] = ''; // 성적서 미제공 항목
    bindings[`수질bod${slotIndex}`] = isTargetDate && row ? (row.bod ?? '') : '';
    bindings[`수질toc${slotIndex}`] = ''; // 성적서 미제공 항목
    bindings[`수질ss${slotIndex}`] = isTargetDate && row ? (row.ss ?? '') : '';
    bindings[`수질tn${slotIndex}`] = isTargetDate && row ? (row.tn ?? '') : '';
    bindings[`수질tp${slotIndex}`] = isTargetDate && row ? (row.tp ?? '') : '';
    bindings[`수질대장균${slotIndex}`] = isTargetDate && row ? (row.total_coliform ?? '') : '';
  };

  setQualitySlot(1, slot1, Boolean(slot1 && normalizeDate(slot1.report_date) === date));
  setQualitySlot(2, slot2, Boolean(slot2 && normalizeDate(slot2.report_date) === date));
