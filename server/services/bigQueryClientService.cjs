...
function getBigQueryClient() {
  ...
  console.log('[BigQuery] 키 파일 경로:', KEY_FILE_PATH);
  console.log('[BigQuery] 키 파일 존재:', fs.existsSync(KEY_FILE_PATH));
  console.log('[BigQuery] DATASET_ID:', DATASET_ID);
  ...
  try {
    ...
    console.log('[BigQuery] 클라이언트 초기화 성공');
    return _client;
  } catch (err) {
