...
/** 기존 회원 행의 id만 교체한다. UUID 전환처럼 참조 키를 바꿀 때 사용한다. */
async function replaceMemberId(oldId, newId) {
  if (!isSheetsConfigured()) throw new Error('Google Sheets가 설정되지 않았습니다.');
  const sheetId = getSheetId();
  await ensureHeader(sheetId);
  ...
  const rowIndex = idCol.indexOf(String(oldId));
  if (rowIndex <= 0) {
    throw new Error(`기존 회원 id를 찾을 수 없습니다: ${oldId}`);
  }
  ...
  await sheets.spreadsheets.values.update({
    ...
    valueInputOption: 'RAW',
    requestBody: { values: [[String(newId)]] }
  });
}
