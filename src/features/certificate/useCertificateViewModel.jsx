...
import { buildCertificateAuthHeaders } from './utils/authUtils';
import { downloadSingleFile, downloadMergedFiles } from './utils/certificateDownloader';
import { deleteFilesProgressive, confirmDelete } from './utils/certificateDeleter';
...
export const useCertificateViewModel = (currentUser, { showToast, showAlert } = {}) => {
    ...
    const [selectedIds, setSelectedIds] = useState(new Set()); // 체크된 항목 ID들
    ...
    const loadRecords = useCallback(async () => {
        ...
        try {
            ...
            // 중앙관리자: 전체 현장 데이터를 가져와 클라이언트에서 필터링
            // (현장 변경 시 깜빡임 방지)
            const siteName = undefined; // 항상 전체 가져오기
            ...
            setRecords(list);
            setSelectedIds(new Set()); // 데이터 로드 시 선택 초기화
            if (list.length === 0) {
                setSelectedId(null);
            } else if (!list.some((item) => item.id === selectedIdRef.current)) {
    ...
    }, [currentUser, showToast, selectedYear, selectedMonth]);
    ...
    const handleDownload = useCallback(async (record) => {
        try {
            await downloadSingleFile(record, showToast);
        } catch (err) {
            // 오류는 모듈 내부에서 처리됨
        }
    }, [showToast]);
    ...
    }, [selectedIds, selectedYear, selectedMonth, selectedSite, showAlert, showToast]);

    const handleDeleteSelected = useCallback(async () => {
        if (selectedIds.size === 0) {
            showAlert?.('삭제할 파일을 선택해 주세요.');
            return;
        }

        const confirmed = confirmDelete(selectedIds.size);
        if (!confirmed) return;
        ...
        // 프로그레시브 삭제 실행
        await deleteFilesProgressive({
            selectedIds,
            records,
            currentUser,
            onProgress: (progress) => setDeleteProgress(prev => ({ ...prev, ...progress })),
            onRecordDeleted: (fileId) => {
                setRecords(prev => prev.filter(r => r.id !== fileId));
            },
    ...
    }, [selectedIds, records, currentUser, showAlert, showToast]);

    // 삭제 진행 다이얼로그 닫기
    const closeDeleteProgress = useCallback(() => {
        setDeleteProgress(prev => ({ ...prev, isOpen: false }));
    }, []);
    ...
    // 선택 상태 계산
    const isAllSelected = visibleRecords.length > 0 && selectedIds.size === visibleRecords.length;
    const isIndeterminate = selectedIds.size > 0 && selectedIds.size < visibleRecords.length;
