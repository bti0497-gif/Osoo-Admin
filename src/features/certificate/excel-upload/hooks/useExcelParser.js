...
export function parseReportDate(dateStr) {
  ...
  // 물결표(~) 또는 대시(-) 기준으로 분할하여 첫 번째 날짜 부분만 획득
  const firstPart = str.split(/[~-]/)[0].trim();
  const currentYear = new Date().getFullYear();

  // 포맷 1: YYYY-MM-DD 또는 YYYY/MM/DD 또는 YYYY.MM.DD
  const dateMatch1 = firstPart.match(/(\d{4})[-./](\d{1,2})[-./](\d{1,2})/);
  if (dateMatch1) {
    return `${dateMatch1[1]}-${String(dateMatch1[2]).padStart(2, '0')}-${String(dateMatch1[3]).padStart(2, '0')}`;
  }

  // 포맷 2: YY-MM-DD 또는 YY/MM/DD 또는 YY.MM.DD (예: 26.01.20)
  const dateMatch2 = firstPart.match(/(\d{2})[-./](\d{1,2})[-./](\d{1,2})/);
  if (dateMatch2) {
    const yr = parseInt(dateMatch2[1], 10);
    const fullYr = yr < 100 ? 2000 + yr : yr;
    return `${fullYr}-${String(dateMatch2[2]).padStart(2, '0')}-${String(dateMatch2[3]).padStart(2, '0')}`;
  }

  // 포맷 3: MM-DD 또는 MM/DD 또는 MM.DD (연도 생략 시 현재 연도 부여)
  const dateMatch3 = firstPart.match(/(\d{1,2})[-./](\d{1,2})/);
  if (dateMatch3) {
    return `${currentYear}-${String(dateMatch3[1]).padStart(2, '0')}-${String(dateMatch3[2]).padStart(2, '0')}`;
  }
...
export function useExcelParser() {
  ...
  const transformToBigQueryFormat = useCallback((rows, siteMaster = []) => {
    ...
    return rows.map(row => {
      ...
      // 인덱스 0: 분석기간(날짜)
      const 분석기간 = String(row[0] ?? '');
      const report_date = parseReportDate(분석기간);
