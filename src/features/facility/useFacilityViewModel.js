...
export const useFacilityViewModel = (currentUser) => {
    ...
    const loadLogs = useCallback(async (q = '') => {
        setLoading(true);
        try {
            const data = await FacilityModel.fetchAll(q || undefined, currentUser);
            setLogs(data);
        } catch (err) {
            console.error(err);
        } finally {
    ...
    const createLog = async (data) => {
        ...
        await FacilityModel.create(data, currentUser);
        await loadLogs(searchQuery);
    };

    const updateLog = async (id, data) => {
        await FacilityModel.update(id, data, currentUser);
        await loadLogs(searchQuery);
    };

    const deleteLog = async (id) => {
        await FacilityModel.remove(id, currentUser);
        await loadLogs(searchQuery);
    };
