...
const MANUAL_PHOTO_ANALYTES = [
    { key: 'nh3_n', label: '암모니아성 질소' },
    { key: 'no3_n', label: '질산성 질소' },
    { key: 'po4_p', label: '오르토 인산염' },
...
const DailyLogStatusDashboard = ({
    ...
    const eligibleDateRows = useMemo(
        () => dashboardDateRows.filter((row) => Number(row.experimentCount || 0) > 0 && Number(row.photoCount || 0) === 0),
        [dashboardDateRows]
    );
    const canShowManualPhotoButton = !isDailyWorkLogDashboard
        && Number(dashboardSummary.totalExperimentCount || 0) > 0
        && Number(dashboardSummary.totalPhotoCount || 0) === 0;

    useEffect(() => {
        if (!eligibleDateRows.length) {
            setSelectedUploadDate('');
            return;
        }
        if (!selectedUploadDate || !eligibleDateRows.some((row) => row.date === selectedUploadDate)) {
            setSelectedUploadDate(eligibleDateRows[0].date);
        }
    }, [eligibleDateRows, selectedUploadDate]);
    ...
    const handleFilePick = (analyteKey, event) => {
        const file = event.target.files?.[0] || null;
        setSelectedFiles((prev) => ({ ...prev, [analyteKey]: file }));
        setPreviewUrls((prev) => {
            if (prev[analyteKey]) {
                URL.revokeObjectURL(prev[analyteKey]);
            }
            return {
                ...prev,
                [analyteKey]: file ? URL.createObjectURL(file) : '',
            };
        });
    };
    ...
    const handleSubmitManualPhotos = async () => {
        if (!selectedUploadDate) return;
        try {
            setUploading(true);
            await onUploadManualPhotos?.({ date: selectedUploadDate, filesByAnalyte: selectedFiles });
            handleCloseModal();
        } catch (_) {
            // 오류 메시지는 ViewModel 토스트에서 처리
        } finally {
            setUploading(false);
        }
    };
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '10px' }}>
                            <div>
                                <div style={{ fontSize: '18px', fontWeight: 800, color: '#0f172a' }}>수질 사진 수동등록</div>
                                <div style={{ fontSize: '12px', color: '#64748b', marginTop: '4px' }}>4개 분석 항목 사진을 각각 선택해 저장하세요.</div>
                            </div>
                            <button
                                type="button"
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <span style={{ fontSize: '12px', fontWeight: 700, color: '#334155' }}>날짜</span>
                            <select
                                value={selectedUploadDate}
                                onChange={(e) => setSelectedUploadDate(e.target.value)}
                                disabled={uploading}
                                style={{ height: '32px', borderRadius: '8px', border: '1px solid #cbd5e1', padding: '0 10px', fontSize: '12px' }}
                            >
                                {eligibleDateRows.map((row) => (
                                    <option key={row.date} value={row.date}>{row.date}</option>
                                ))}
                            </select>
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '12px' }}>
                            {MANUAL_PHOTO_ANALYTES.map((item) => (
                                <label
                                    key={item.key}
                                    style={{
                                        border: '1px solid #e2e8f0',
                                ...
                                >
                                    <span style={{ fontSize: '12px', fontWeight: 800, color: '#1e293b' }}>{item.label}</span>
                                    <input
                                        type="file"
                                        accept="image/*"
                                        disabled={uploading}
                                        onChange={(event) => handleFilePick(item.key, event)}
                                        style={{ fontSize: '12px' }}
                                    />
                                    <div style={{
                                        height: '86px',
                                        ...
                                        {previewUrls[item.key] ? (
                                            <img src={previewUrls[item.key]} alt={`${item.label} 미리보기`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                        ) : (
                                            <span style={{ fontSize: '11px', color: '#94a3b8' }}>미리보기 없음</span>
                                        )}
                                    </div>
                                </label>
                            ))}
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                            ...
                            <button
                                type="button"
                                onClick={handleSubmitManualPhotos}
                                disabled={uploading}
                                style={{
                                    border: 'none',
                                    background: '#1e293b',
