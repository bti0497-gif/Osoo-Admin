...
export const useDailyLogViewModel = (currentUser, initialDate, templateName, showToast) => {
    ...
    }, [selectedDates, computedStartDate, computedEndDate, templateName, siteName, refreshNonce]);
    ...
    }, [currentPage, computedEndDate, computedStartDate, templateName, siteName, refreshNonce]);
    ...
    }, [pages, currentPage, pageRenderData, computedStartDate, computedEndDate, templateName, siteName, expectedPhotoCountPerSheet, isDailyWorkLog, refreshNonce]);
    ...
    const uploadManualWaterPhotos = async ({ date, filesByAnalyte = {} }) => {
        if (!date) {
            throw new Error('업로드 날짜가 필요합니다.');
        }

        const analyteEntries = Object.entries(filesByAnalyte).filter(([, file]) => Boolean(file));
        if (!analyteEntries.length) {
            throw new Error('업로드할 사진을 하나 이상 선택해 주세요.');
        }

        const targetRow = dashboardRows.find((row) => row.date === date);

        try {
            for (const [analyte, file] of analyteEntries) {
                const result = await DailyLogModel.uploadManualWaterPhoto({
                    date,
                    analyte,
                    file,
                    measurementGroup: targetRow?.measurementGroup || '',
                    measurementOrder: targetRow?.measurementOrder ?? 1,
                });
                if (!result?.success) {
                    throw new Error(result?.error || '수동 사진 저장에 실패했습니다.');
                }
            }

            refreshDashboard();
            showToast?.(`사진 ${analyteEntries.length}건을 저장했습니다.`);
            return { success: true, uploadedCount: analyteEntries.length };
        } catch (error) {
            showToast?.(error.message || '수동 사진 저장에 실패했습니다.', 'error');
            throw error;
        }
    };
