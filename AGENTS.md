...
- 회원/현장 관리 목록 조회 시 10명 제한 등 임의의 페이지네이션 슬라이스(`slice`)를 금지하고, 전체 검색 결과 목록(`filteredMembers`)을 전달하여 스크롤 그리드에 모든 회원이 조회되도록 보존한다.
- 일렉트론 설치판/패키징 환경에서 작업 폴더/임시 파일/서버 로컬 데이터는 권한 제한이 없는 `appData` (`app.getPath('userData')` / `%APPDATA%` 하위 경로)를 반드시 사용한다. Program Files 등 권한이 없는 디렉토리에 작업 폴더 생성을 시도하지 않는다.
...
- 정산 보고서 생성 타이밍: 매월 말일 ~ 익월 2일 (말일 자정 검침 집계 후 도로공사 마감 제출). 정산 화면 진입 시 한글 COM 백그라운드 사전 웜업(Pre-warm) 및 화면 갱신 락(LockCommand)으로 초고속 바인딩을 보장한다.
- 정산서 자동 생성 규칙(청주 HWP, 천안부산 XLSX, 죽암부산 XLS, 죽암서울 XLS, 홍천양양 XLSX, 홍천양양 HWP)은 각 현장 고유 규격으로 검증 완료되었으므로 임의로 수정/단순화하지 않는다. 상세 규격은 `.agents/AGENTS.md` 제10조를 준수한다.
- 정산서 Excel COM/HWP COM 작업 시 `$null` 금지(`""` 사용), `.Value2` 사용, `$rng.MergeArea` 기준 정중앙 정렬, 생성 완료 시 `월정산 > {현장마감자료} > YYYYMM` 및 바탕화면 루트 동시 복사를 보존한다.
